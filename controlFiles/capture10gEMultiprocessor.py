...
def udpHandler(dataQ):
				...
				#amd bind to the address and ports that we have set up on the roach
				...
				while True:
								...
								# we will unpack the data we have received so we can looka t it directly before it goes ont the multiproc heap. Note that this is the full data packet we are looking at
							...
							#	print oldTime,newTime
							  ##now we have 32 packets per  2048 pt fine FFT. When we have received all of those we will put the data in the queue
								if iCount>=32:
...
def dataProcessor(dataQ):
				...
				while(1):
								...
								ttimag = ((tt&0x0f))
								ttreal = (((tt>>4)&0x0f))
								ttimag[ttimag>=8]=ttimag[ttimag>=8]-16
